...
STOPWORDS = [
  ...
  "you'll", "you're", "you've", "your", "yours", "yourself", "yourselves", "zero", "—", "♫", "(laughter)",
  "(applause)"
].freeze

STOPWORDS.each do |w|
  normalized_word = w.strip.downcase
  word = Word.find_by(normalized_text: normalized_word)

  unless word.nil?
    word.is_stop_word = true
    word.save
    puts "Tagged '#{normalized_word}' as a stop word."
  end
end
